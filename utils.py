...
def set_params(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
    ...
    alpha_ca = 1000
    alpha_obst = 5e3
    n_xi = np.array([8, 8, 8, 8])  # \xi dimension -- number of states of REN
    l = np.array([8, 8, 8, 8])  # dimension of the square matrix D11 -- number of _non-linear layers_ of the REN
    n_traj = 4  # number of trajectories collected at each step of the learning
    std_ini = 0.1  # standard deviation of initial conditions
